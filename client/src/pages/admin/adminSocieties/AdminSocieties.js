...
// constants
import { SOCIETIES_GET_ENDPOINT, SOCIETIES_POST_ENDPOINT, SOCIETIES_PATCH_ENDPOINT, SOCIETIES_DELETE_ENDPOINT } from "../../../constants/endpoints";
// contexts
...
const AdminSocieties = () => {
    ...
    const [societiesEvents, setSocietiesEvents] = useState([]);
    ...
    const handleUpload = (data, files, mode) => {
        ...
        if (files)
            ...
            axios.patch(SOCIETIES_PATCH_ENDPOINT, data)
                .then(res => {
                    setIsLoading(false);
                    setExpanded(false);
                    listRef.current.scrollTo(0, 0);
                    setSocietiesEvents(events => {
            ...
            axios.post(SOCIETIES_POST_ENDPOINT, data)
                .then(res => {
                    setSocietiesEvents(societiesEvents => [res.data.doc, ...societiesEvents]);
                    setIsLoading(false);
                    setExpanded(false);
                    setNewMediaFiles([]);
                    listRef.current.scrollTo(0, 0);
    ...
    const getFileURLs = (data, files, mode) => {
        ...
        files.map((file, index) => {
            const storageRef = ref(storage, "societiesEvents/" + file.name);
    ...
    const handleDelete = _id => {
        setIsLoading(true);
        axios.delete(SOCIETIES_DELETE_ENDPOINT + "/" + _id)
            .then(res => {
                const { _id } = res.data;
                setSocietiesEvents(societiesEvents => {
                    let myEvents = societiesEvents.filter(event => event._id !== _id);
                    return myEvents;
                });
                setExpanded(false);
                setIsLoading(false);
        ...
        <div className="adminSocieties">
            ...
            {canDelete._id ? <Dialog open={true} onClose={() => setCanDelete({})}>
                <div className="adminSocieties__alertBox">
                    ...
                    <div className="adminSocieties__alertButtons">
                        <Button onClick={() => setCanDelete({})}>Cancel</Button>
                        <Button onClick={() => handleDelete(canDelete._id)}>Delete</Button>
                    </div>
                </div>
            </Dialog> : null}
            <div className="adminSocieties__new">
                <h1>New Society Event</h1>
                <p>Add a society event to the website.</p>
                <form onSubmit={e => handleForm(e, "post", [])} ref={newFormRef}>
                    ...
                    <TextareaAutosize id="adminSocieties__description" placeholder="Description" name="description" />
                    <div className="adminSocieties__group">
                        <FormControl>
                            <InputLabel variant="standard" htmlFor="adminSocieties__selectCategory">Category</InputLabel>
                            <NativeSelect inputProps={{ name: "category", id: "adminSocieties__selectCategory", }} defaultValue={"Technology"} style={{ marginRight: "5px" }}>
                        ...
                        <FormControl>
                            <InputLabel variant="standard" htmlFor="adminSocieties__selectSubCategory">Sub-Category</InputLabel>
                            <NativeSelect defaultValue="NA" inputProps={{ name: "subCategory", id: "adminSocieties__selectSubCategory" }} style={{ marginLeft: "5px" }}>
                    ...
                    <div className="adminSocieties__buttons">
                        <div className="adminSocieties__filesInput" style={{ marginRight: "2.5px" }}>
            ...
            <div className="adminSocieties__old">
                <div className="adminSocieties__head">
                    <div className="adminSocieties__title">
                        <h1>Societies' Events List</h1>
                        <p>List of Societies' Events around the campus.</p>
                    </div>
                    <form className="adminSocieties__search" onSubmit={e => handleSearch(e)}>
                        <TextField variant="standard" label="Search" name="search" />
                        <IconButton type="submit"><SearchRoundedIcon /></IconButton>
                    </form>
                </div>
                <div className="adminSocieties__list" ref={listRef}>
                    {societiesEvents.filter(event => getSearchStr(event).includes(search.toLowerCase())).map((event, index) =>
                        <form key={event._id} onSubmit={e => handleForm(e, "patch", event.updatesBy)}>
                            <Accordion expanded={expanded === "panel" + (index + 1)} onChange={handleChange("panel" + (index + 1))} TransitionProps={{ unmountOnExit: true }}>
                                ...
                                <AccordionDetails>
                                    ...
                                    <p style={{ marginBottom: "5px" }}>Description</p>
                                    <TextareaAutosize id="adminSocieties__description" placeholder="Description" name="description" defaultValue={event.description} />
                                    <div className="adminSocieties__group">
                                        <FormControl>
                                            <InputLabel variant="standard" htmlFor="adminSocieties__editCategory">Category</InputLabel>
                                            <NativeSelect defaultValue={event.category} inputProps={{ name: "category", id: "adminSocieties__editCategory" }} style={{ marginRight: "5px" }}>
                                                <option value="Technology">Technology</option>
                                                <option value="Social And Culture">Social And Culture</option>
                                                <option value="Sports And Games">Sports And Games</option>
                                                <option value="Students' Welfare">Students' Welfare</option>
                                        ...
                                        <FormControl>
                                            <InputLabel variant="standard" htmlFor="adminSocieties__editSubCategory">Sub Category</InputLabel>
                                            <NativeSelect defaultValue="NA" inputProps={{ name: "subCategory", id: "adminSocieties__editSubCategory" }} style={{ marginLeft: "5px" }}>
                                                <option value="NA">Not Applicable</option>
                                            </NativeSelect>
                                        </FormControl>
                                    </div>
